    ...
    <h6 className="border-bottom border-gray pb-2 mb-0"></h6>
        {currentUserTrips.length ? (
         currentUserTrips.map(trip => {
          return (
            <div className="media text-muted pt-3 row">
                <div className="media-body pb-3 mb-0 small lh-125 border-bottom border-gray justify-content-between align-items-center">
                  <button className="shadow trip-btn rounded" style={{ color: 'primary', textDecoration: 'none',}} onClick={handleSubmit} value={trip._id}>
                    <div className="row align-items-center" style={{ margin: '5px',}}>
                      <img className="" src={logo} style={{height: "60px", width:"auto"}}></img>
                      <div className="col ">
                        <strong className="d-block text-gray-dark">
                          <h4 className="display-6">{trip.tripName}</h4>
                        </strong>
                        <strong className="lead d-block text-gray-dark">{trip.destination}  ({(moment(trip.startDate).format('MMMM Do'))} - {(moment(trip.endDate).format('Do, YYYY'))})</strong>
                    </div>
                    </div>
                  </button>
                  <button className="pt-1 pb-1 rounded btn-color ml-5 flex-end" onClick={() => generatePDF(trip)}>
